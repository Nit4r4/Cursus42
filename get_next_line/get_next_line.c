...
/*   Updated: 2021/12/13 19:15:20 by lgenevey         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{	// utiliser une variable preprocesseur est ok : buffer[BUFFER_SIZE + 1]
	...
	static char	*leftover; // garde les laisses pour compte
	...
	while (!has_symbol(leftover, '\n'))
	{
		//remplir buffer et stocker le retour de read()
		been_read = read(fd, buffer, BUFFER_SIZE);
		// check si erreur avec read() et on libere le tableau avant de retourner NULL
		// on peut pas faire avant, que apres avoir appele read()
		if (has_symbol(leftover, '\0'))
		{
			printf("coucou");
		}
		if (been_read < 1)
	...
	free(buffer);
	//buffer = NULL;
	// inscrire le contenu de leftover jusqu'a \n dans line
	//buffer = leftover;
